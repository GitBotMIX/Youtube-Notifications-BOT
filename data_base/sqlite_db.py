...
from constants import YOUTUBE_TABLE, YOUTUBE_ROWS, USER_TABLE, USER_ROWS, REMIND_TABLE, REMIND_ROWS
...
def sql_start():
    ...
    base.execute(f'CREATE TABLE IF NOT EXISTS {USER_TABLE}({USER_ROWS["STATUS"]} TEXT, {USER_ROWS["LANGUAGE"]} TEXT, '
                 f'{USER_ROWS["TIMEZONE"]}, {USER_ROWS["USER"]} TEXT)')
    ...
    base.execute(f'CREATE TABLE IF NOT EXISTS '
                 f'{REMIND_TABLE}({REMIND_ROWS["VIDEO_URL"]} TEXT, '
                 f'{REMIND_ROWS["JOB_TIME"]} TEXT, {REMIND_ROWS["USER"]} TEXT)')
    base.commit()
    return cur, base
...
class Methods:
    ...
    async def get_all_rows_related_reverse(cls, *related):
        if len(cls.SELECT_ROWS) == 1:
            ...
            __select_rows_str = ' == ? AND  '.join(cls.SELECT_ROWS)
    ...
    async def where_user(cls, user_id):
        __where_user_tuple_list = cur.execute(f'SELECT {cls.SELECT_ROW} '
                                              f'FROM {cls.TABLE} '
                                              f'WHERE {cls.WHERE_ROW} == ?', (user_id,)).fetchall()
        return await tuple_list_to_list_first_values_in_tuple(__where_user_tuple_list)

    @classmethod
    async def get_all(cls):
        __get_all_tuple_list = cur.execute(f'SELECT {cls.SELECT_ROW} '
                                           f'FROM {cls.TABLE}').fetchall()
        return await tuple_list_to_list_first_values_in_tuple(__get_all_tuple_list)
...
class User:
    ...
    class Timezone(Methods):
        SELECT_ROW = USER_ROWS["TIMEZONE"]
        TABLE = USER_TABLE
        WHERE_ROW = USER_ROWS["USER"]

        @classmethod
        async def update(cls, timezone: str, user_id: str | int):
            await Database.sql_update(cls.TABLE, cls.SELECT_ROW, timezone, cls.WHERE_ROW, str(user_id))
...
class Remind(Methods):
    SELECT_ROWS = [REMIND_ROWS["VIDEO_URL"], REMIND_ROWS["JOB_TIME"], REMIND_ROWS["USER"]]
    TABLE = REMIND_TABLE

    @staticmethod
    async def add(*args: str):
        await Database().sql_add(args, REMIND_TABLE)

    @staticmethod
    async def delete(user_id: str | int):
        await Database().sql_delete(REMIND_ROWS, REMIND_ROWS['USER'], str(user_id))

    class VideoUrl(Methods):
        TABLE = REMIND_TABLE
        SELECT_ROW = REMIND_ROWS["VIDEO_URL"]
        SELECT_ROWS = [REMIND_ROWS["VIDEO_URL"], REMIND_ROWS["USER"]]
        WHERE_ROW = REMIND_ROWS["USER"]

        @classmethod
        async def delete(cls, video_id: str, user_id: str | int):
            await Database().sql_delete(cls.TABLE, cls.WHERE_ROW, str(user_id),
                                        video_url=video_id)

        @classmethod
        async def update(cls, video_url: str, job_time: str, user_id: str | int):  # update job time
            await Database.sql_update(cls.TABLE, REMIND_ROWS['JOB_TIME'], job_time, cls.WHERE_ROW, str(user_id),
                                      video_url=video_url)
